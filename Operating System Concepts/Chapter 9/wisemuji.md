...
- 연속적인 메모리 할당의 두 가지 문제를 해결한다.
- 물리적 메모리를 고정 크기의 블록(프레임)으로 분할, 논리 메모리를 같은 크기의 블록(페이지)으로 분할한다.
- 즉, 논리적 주소 공간을 물리적 주소 공간과 완전히 분리한다.

### 기본적인 구현 방법

CPU에서 생성된 모든 주소는 페이지 번호와 오프셋으로 나뉜다. 페이지 번호는 프로세스별 페이지 테이블의 인덱스로 사용된다.
<img width="866" alt="image" src="https://github.com/Coveong/reading-books-for-programmers/assets/32327475/b63464c0-3aff-4b26-be98-9a814c3c7258">

- 페이지 크기(프레임 크기와 동일)는 하드웨어에 의해 정의된다.
- 논리적 주소 공간의 크기가 2의 m제곱이고 페이지 크기가 2의 n제곱이면 상위 m-n 비트가 페이지 번호를 지정하고 하위 n 비트가 페이지 오프셋을 지정한다.

<img width="900" alt="image" src="https://github.com/Coveong/reading-books-for-programmers/assets/32327475/67127141-224e-483d-b263-a3bfdd01bc7e">

## PTBR
페이지 테이블 기본 레지스터는 페이지 테이블을 가리키며 페이지 테이블은 기본 메모리에 유지된다.
- 컨텍스트 전환 속도는 빨라지지만 메모리 액세스 시간은 느려진다. 
- 두 개의 메모리 액세스가 필요하다. (테이블 항목용, 실제 데이터용)
