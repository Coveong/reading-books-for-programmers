...
웹 서버가 1분, 1시간 동안 얼마나 많은 바이트를 전송했는지 추적할 필요가 있다.
...
2. MinuteCount()와 HourCount()가 너무 느리다.
   - 시간범위에 있는 데이터 포인트의 개수가 n일 때 O(n)만큼 시간을 소모하는데 초당 100add()를 한다고 했을때 HourCount가 호출되면 100만데이터 처리해야 해 느리다.

> 코드는 책에 있는데 길고 정리하기가 어려워서 책 참고
...
1. 필요 없는 데이터를 삭제하라
2. 미리 계산된 minute_count와 hour_count의 총합이 가장 최근에 계산된 값을 담게 하라

문제

1.설계가 유연하지 않다.

2.자주 호출된 만큼 더 많은 메모리가 소모된다.
...
코드의 길이는 길어지지만 다재다능하고 깔끔하다.



- **ConveyorQueue**: 시스템이 허락하는 한도 내에서 무제한의 크기를 갖는 큐로 총합관리
- **TrailingBUcketCounter**: 시간의 흐름에 따라서 ConveyorQueue를 옆으로 한 칸 이동시키고, 시간구간의 카운트를 저장한다.
- **MinuteHourCounter** : TrailingBucketCounter 두개 보관한다. 하나는 1분 하나는 1시간에 대한 카운트 용



#### 성능통계 수치

| 해결책        | 코드 줄 수 | HourCount()      | 메모리사용량    | HourCount() Error |
| ------------- | ---------- | ---------------- | --------------- | ----------------- |
| 순진한 해결책 | 33         | O(시간당 이벤트) | infinite        | 3,600 당 1개      |
| 컨베이어 벨트 | 55         | O(1)             | O(시간당 event) | 3,600 당 1개      |
| 시간바구니    | 98         | O(1)             | O(바구니수)     | 60 당 1개         |
