...
- 불필요한 임시 변수를 사용하지 말아라.

이런변수는 코드 수정 후 우연히 남겨진 '부산물'인 경우가 많다.



- 중간 결과를 삭제하라.

중간 결과를 저장하는 변수 대신 곧바로 처리하는 방식으로 처리할수 있는 경우가 많다.
...
이런 변수는 실제 데이터를 저장하지 않는다.

흐름 제어 변수는 프로그램 구조를 잘 설계하면 제거할 수 있는 경우가 있다.



### 변수의 범위를 좁혀라

- 핵심아이디어 : 변수가 적용되는 범위를 최대한 좁게 만들어라

이유는?

코드를 읽는 사람이 생각해야 하는 변수 수를 줄여주기 때문이다.



- 많은 멧드를 정적 static으로 만들어서 클래스 멤버 접근을 제한하라.

코드를 읽는 사람에게 '이 코드는 저 변수들로부터 독립적'이라는 사실을 알려줄 수 있다.
...
최선의 실전방법은 변수를 (ex) var x = 1)항상 var 키워드와 함께 선언하는 것이다.
...
c++에서는 const , Java에서는 final을 사용하는것도 좋다.
...
- **방해되는 변수를 제거하라.** 결과를 즉시 처리하는 방식으로 '중간 결과값'을 저장하는 변수를 제거
- 각 변수의 범위를 최대한 작게 줄여라. 각 변수의 위치를 옮겨서 변수가 나타나는 줄의 수를 최소화 하라. 눈에 안보이면 멀어진다.
- **값이 한 번만 할당되는 변수를 선호하라**. 값이 한번만 할당되는(const,final 혹은 불변인)변수는 이해하기 쉽다.
